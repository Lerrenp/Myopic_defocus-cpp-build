...
*   **🖥️ 桌面复制 API**：使用 Windows 的 `Desktop Duplication API`，支持 HDR 和高色深（10bit）格式（目前仅支持输入不报错，未实现电平校准和hdr输出，建议关闭hdr），兼容 AMD/NVIDIA 独显。
...
*   Visual Studio 2022（安装 VS 时勾选 C++ 桌面开发）
*   Windows 11 SDK
...
> **关于退出**：焦点丢失后可以使用任务管理器（Taskmgr）直接结束进程，如不喜欢esc可以自行定义其他vk虚拟键码，或者使用键盘钩子。
