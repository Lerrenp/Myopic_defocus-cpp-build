Myopic Defocus Simulator (C++ / DirectX 11 High Performance)
近视散焦模拟器 - C++ 高性能重构版
本项目是将原本基于 Python 的近视散焦模拟工具重构为原生 C++ 版本。通过使用 DirectX 11 和 DXGI Desktop Duplication API，实现了极低的延迟和极高的性能，解决了 Python 版本在高帧率屏幕下的卡顿和高 CPU 占用问题。
✨ 核心特性 (Features)
...
👁️ 物理光学模拟: 模拟纵向色差 (Longitudinal Chromatic Aberration, LCA)。不同于普通的高斯模糊，本程序对蓝光和绿光通道分离计算模糊半径，以模拟近视防控中的离焦效果。
...
Visual Studio 2022
Windows SDK (安装 VS 时勾选 C++ 桌面开发即可)
步骤
创建项目: 在 Visual Studio 中创建一个新的 Windows 桌面应用程序 (Windows Desktop Application) 项目 (C++)。
设置标准: 在项目属性中，将 C++ 语言标准设置为 ISO C++20（或 C++17）。
添加依赖: 在 项目属性 -> 链接器 -> 输入 -> 附加依赖项 中，确保包含以下库：
code
Text
d3d11.lib
dxgi.lib
...
dwmapi.lib
windowsapp.lib
DPI 感知: 在 项目属性 -> 清单工具 -> 输入和输出 -> DPI 感知 中选择 "Per Monitor High DPI Aware" (或者代码中已通过 SetProcessDpiAwarenessContext 设置，此处可跳过)。
编译: 选择 Release 和 x64 模式进行编译。
...
struct Config {
    ...
    float diagInch = 27.0f; 

    // 观看距离 (厘米)
    float screenDistanceCM = 60.0f;
...
⚠️ 免责声明 (Disclaimer)
本软件仅供学习和实验用途。虽然其基于近视离焦原理编写，但不构成任何医疗建议。使用过程中如感到眼部不适（如头晕、恶心），请立即停止使用。
